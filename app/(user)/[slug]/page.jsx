...
// =============================================
// HELPER: Scroll dengan offset untuk sticky navbar
// =============================================
const scrollToRef = (ref, offset = 72) => {
...
const GamesTopup = () => {
  ...
  // Tinggi navbar sticky (sesuaikan jika berbeda)
  // Navbar h-16 (64px) + submenu h-12 (48px) = 112px total
  ...
  const step2Ref = useRef(null); // Pilih Nominal
  const step3Ref = useRef(null); // Payment
  const step4Ref = useRef(null); // Data Pembeli
  ...
  // =============================================
  // AUTO SCROLL — pakai scrollToRef (dengan offset navbar)
  // =============================================

  // Scroll ke Step 2 setelah input akun valid (non-PLN)
  useEffect(() => {
    if (!isPlnProduct && isAccountComplete()) {
      setTimeout(() => scrollToRef(step2Ref, NAVBAR_HEIGHT), 200);
    }
  }, [accountData]);

  // Scroll ke Step 2 setelah PLN inquiry sukses
  useEffect(() => {
    if (plnData) {
      setTimeout(() => scrollToRef(step2Ref, NAVBAR_HEIGHT), 200);
    }
  ...
  Object.keys(groupedProducts).forEach((category) => {
    groupedProducts[category].sort(
      (a, b) => Number(a.selling_price) - Number(b.selling_price),
    );
  });

  useEffect(() => {
    if (products.length > 0 && !activeTab) {
      const firstType = products[0].category || "";
  ...
  const ProductCard = ({
    ...
    return (
      <div
        ...
        onClick={() => {
          onSelect(product);
          // ✅ Pakai scrollToRef dengan offset navbar
          setTimeout(() => scrollToRef(step3Ref, NAVBAR_HEIGHT), 150);
        }}
        style={{
  ...
  const renderProducts = () => {
    ...
    if (!products.length) {
      return <div className="text-center text-gray-400">Tidak ada produk</div>;
    }
  ...
  const calculateTotalFee = () => {
    ...
    switch (paymentMethod.fee_type) {
      case "flat":
        return Number(paymentMethod.nominal_fee) || 0;

      case "percentage":
        if (paymentMethod.percentase_fee > 0) {
          return (price * paymentMethod.percentase_fee) / 100;
        }
        return 0;
      ...
      case "mixed":
        let percentFee = 0;
        if (paymentMethod.percentase_fee > 0) {
          percentFee = (price * paymentMethod.percentase_fee) / 100;
        }
        return (Number(paymentMethod.nominal_fee) || 0) + percentFee;
  ...
  return (
    <div className="min-h-screen bg-[#37353E]">
      ...
      <div className="flex justify-center gap-2 md:hidden mt-2">
        <button
          ...
          className={`px-4 py-2 rounded ${
            pilih === 0 ? "bg-gray-800" : "bg-gray-600"
          }`}
        >
          Topup
        </button>
        ...
        <button
          onClick={() => setPilih(0)}
          className={`px-4 py-2 rounded ${
            pilih === 1 ? "bg-gray-800" : "bg-gray-600"
          }`}
        >
          Keterangan
        </button>
      ...
      <div className="container mx-auto px-4 max-w-7xl py-6">
        {pilih == 0 ? (
          <div
            className="rounded-2xl shadow-lg overflow-hidden sticky top-6 mb-2 mt-2 md:mt-0"
            ...
            <form onSubmit={handleSubmit}>
              <div className="lg:flex lg:space-x-6">
                                ...
                                {methods.map((method) => (
                                  <div
                                    ...
                                    onClick={() => {
                                      ...
                                      setTimeout(
                                        () =>
                                          scrollToRef(step4Ref, NAVBAR_HEIGHT),
                                        150,
                                      );
                                    }}
                                    style={{
                ...
                <div className="lg:w-4/12">
                  <div
                    className="rounded-2xl shadow-lg overflow-hidden sticky top-6 mb-2 mt-2 md:mt-0 hidden md:flex"
                    style={{ backgroundColor: COLORS.primary }}
                  >
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-gray-100 mb-4">
                  ...
                  <div
                    className="rounded-2xl shadow-lg overflow-hidden sticky top-6 mt-4 md:mt-0"
                    style={{ backgroundColor: COLORS.primary }}
                  >
                    <div className="p-6">
                      <h3 className="text-xl font-bold text-gray-100 mb-4">
